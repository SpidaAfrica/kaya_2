...
const QuickButton = memo(({ text, onClick }: QuickButtonProps): React.ReactElement => (
  <Button variant="secondary" className="min-w-[10rem]" onClick={onClick}>
    {text}
  </Button>
));

function Message({ sender, content, timestamp }: { sender: string; content: string; timestamp?: string }): React.ReactElement {
  return (
